        ...
        //if (!tile.isDud) {
          if (next && next.value === tile.value && !next.mergedFrom) {
              ...
              merged.unstable = merged.value === 8
                             || merged.value === 32
                             || merged.value === 128
                             || merged.value > 256 ? (merged.value > 8 ? merged.value : 5) : 0;
  ...
  if (moved) {
    ...
    for (i = 0; i < unstableTiles.length; i++) {
      ...
      if (unstableTile.unstable === 1) {
        ...
        // 爆炸时减半方块值
        unstableTile.value /= 2;
      
        // 重置不稳定状态
        if (unstableTile.value > 3) {
            unstableTile.unstable = 2;
        } else if (unstableTile.value < 3) {
            unstableTile.unstable = 1;
            unstableTile.explode = true;
            exploded = true;
        }
      
        // 添加特殊效果动画
        unstableTile.highlight = "halve";
      } else if (unstableTile.unstable > 1) {
